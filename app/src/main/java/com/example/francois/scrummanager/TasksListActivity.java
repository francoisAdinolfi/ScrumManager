        ...
        if(session.getUserDetails().get(SessionManager.KEY_ROLE).equals("scrummaster")) {
            ...
            btnScheduling.setOnClickListener(v -> {
                if(schedulingDone) {
                    ...
                    if(tasksId.isEmpty()){
                        Toast.makeText(TasksListActivity.this, "No task", Toast.LENGTH_LONG).show();
                    } else if (devsId.isEmpty()){
                        Toast.makeText(TasksListActivity.this, "No developer", Toast.LENGTH_LONG).show();
                    } else{
                        Schedule schedule = solver(tasksId, durations, dependencies, devsId, unavailabilitiesDay);
                        if (schedule != null) {
                            addSchedule(schedule);
                        } else {
                            Toast.makeText(TasksListActivity.this, "No Solution", Toast.LENGTH_LONG).show();
                        }
                    }
                }
            });
